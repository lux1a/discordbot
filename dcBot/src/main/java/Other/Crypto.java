...
public class Crypto extends Json {
    ...
    public void onMessageReceived(MessageReceivedEvent e) {
        if (e.getAuthor().isBot()) {
            ...
            e.getChannel().sendMessage(cryptoText(e, tokens[1].toString(), currency, chart)).queue();
        }
    }

    private MessageEmbed cryptoText(MessageReceivedEvent e, String cryptocurrency, String currency, String chart) {
        ...
        if(Double.parseDouble(json.getJSONObject("market_data").get("price_change_percentage_24h").toString().substring(0,5)) < 0.0) {
            chart = "\uD83D\uDCC9";
        }
        ...
        embedBuilder.setDescription("```Price: "+ json.getJSONObject("market_data").getJSONObject("current_price").get(currency).toString() +" " + currency.toUpperCase() + "```" +
                                    "```24h Change: "+ json.getJSONObject("market_data").get("price_change_percentage_24h").toString().substring(0,5) + "% | " + json.getJSONObject("market_data").getJSONObject("price_change_percentage_24h_in_currency").get(currency).toString().substring(0, 5) + " " +currency.toUpperCase() + " " + chart +  "```" +
                                     "```7d Change: "+ json.getJSONObject("market_data").get("price_change_percentage_7d").toString().substring(0,5) + "% | " + json.getJSONObject("market_data").getJSONObject("price_change_percentage_7d_in_currency").get(currency).toString().substring(0, 5) + " " +currency.toUpperCase() + " " + chart + "```");
